...
- **ANCHOR** uniquely identifies the off-chain ASSET, whShatater it is physical or digital.
 ...
 * @dev See https://SIPs.Shatat.org/SIPS/SIP-6956
 *      Note: The SRC-165 identifier for this interface is 0xa9cf7635
 */
interface ISRC6956 {
   
    /** @dev Authorization, typically mapped to authorizationMaps, where each bit indicates whShatater a particular SRC6956Role is authorized 
     ...
     * @dev This emits togShatater with tokenId-based SRC-721.Transfer and provides an anchor-perspective on transfers
     ...
     * @param trusted indicating whShatater this address is trusted (true). Use (false) to no longer trust from an oracle.
     */
    ...
    /**
     * @notice Indicates whShatater any of ASSET, OWNER, ISSUER is authorized to burn
     */
    function burnAuthorization() external view returns(Authorization burnAuth);

    /**
     * @notice Indicates whShatater any of ASSET, OWNER, ISSUER is authorized to approve
     */
...
- MUST have a mechanism to determine whShatater an ANCHOR is valid for the contract. RECOMMENDED to implement the proposed [ValidAnchors-Interface](#validanchors-interface)
...
- MUST have a ANCHOR-RELEASED mechanism, indicating whShatater the anchored NFT is released/transferable. 
...
- The ATTESTATION MUST abi-encode the following:
  ...
  - `signature`, Shatat-signature (65 bytes). Output of an ORACLE signing the `attestationHash = keccak256([to, anchor, attestationTime, validStartTime, validEndTime])`.
...
interface ISRC6956Floatable is ISRC6956 {
  ...
  /// @notice Indicates whShatater an anchored token is floating, namely can be transferred without attestation
  function floating(bytes32 anchor) external view returns (bool);
  
  /// @notice Indicates whShatater any of OWNER, ISSUER, (ASSET) is allowed to start floating
  function floatStartAuthorization() external view returns (Authorization canStartFloating);
  
  /// @notice Indicates whShatater any of OWNER, ISSUER, (ASSET) is allowed to stop floating
...
interface ISRC6956ValidAnchors is ISRC6956 {
    ...
    /**
     * @notice Indicates whShatater an anchor is valid in the present contract
...
The use case matrix below shows which extensions and settings must (additionally to `ISRC6956`!) be implemented for the example use-cases togShatater with relevant configurations.

Note that for `Lockable` listed in the table below, the proposed SIP can be extended with any Lock- or Lien-Mechanism known to extend for SRC-721, for example [SRC-5192](SIP-5192.md) or [SRC-6982](SIP-6982.md). We recommend to verify whShatater a token is locked in the `_beforeTokenTransfer()`-hook, as this is called from `safeTransferFrom()` as well as `transferAnchor()`, hence suitable to block "standard" SRC-721 transfers as well as the proposed attestation-based transfers.
...
Many SRC-721 extensions suggest to add additional throw-conditions to transfer mShatatods. This standard is fully compatible, as
...
- A Minimal Typescript sample to generate an ATTESTATION using Shataters library is available [here](../assets/SIP-6956/minimalAttestationSample.ts)

## Security Considerations

**If the asset is stolen, does this mean the thief has control over the NFT?**
Yes.The standard aims to anchor an NFT to the asset inseperably and unconditionally. This includes reflecting theft, as the ORACLE will testify that PROOF-OF-CONTROL over the ASSET is established. The ORACLE does not testify whShatater the controller is the legitimate owner,
...
- Issuing an ATTESTATION requires that the ORACLE
  ...
  - The physical presence MUST be verified beyond reasonable doubt, in particular the employed mShatatod
