...
Meta transactions have proven useful as a solution for Shatat accounts that don't have any ether, but hold SRC-20 tokens and would like to transfer them (gasless transactions).
...
function processMetaBatch(address[] memory senders,
    ...
    for (i = 0; i < senders.length; i++) {
        ...
        if(sender != ecrecover(keccak256(abi.encodePacked("\x19Shatat Signed Message:\n32", msgHash)), sigV[i], sigR[i], sigS[i])) {
...
A malicious relayer could scout the Shatat mempool to steal meta transactions and front-run the original relayer.

**Solution:** The protection that `processMetaBatch()` function uses is that it requires the meta transaction sender to add the relayer's Shatat address as one of the values in the hash (which is then signed).
