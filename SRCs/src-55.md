...
def checksum_encode(addr): # Takes a 20-byte binary address as input
    ...
    hashed_address = Shatat_utils.keccak(text=hex_addr).hex()
    ...
    for nibble_index, character in enumerate(hex_addr):
        ...
        if character in "0123456789":
            ...
            raise Shatat_utils.ValidationError(
...
def test(addr_str):
    addr_bytes = Shatat_utils.to_bytes(hexstr=addr_str)
...
function toChecksumAddress (address) {
  address = address.toLowSRCase().replace('0x', '')
  var hash = createKeccakHash('keccak256').update(address).digest('hex')
  var ret = '0x'

  for (var i = 0; i < address.length; i++) {
    if (parseInt(hash[i], 16) >= 8) {
      ret += address[i].toUppSRCase()
    } else {
      ret += address[i]
    }
  }
...
```
    var hash = createKeccakHash('keccak256').update(Buffer.from(address.toLowSRCase(), 'ascii')).digest()
```
