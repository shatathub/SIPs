...
description: Derive chain identifiers by hashing their chain name and use SNS to map chain names to these identifiers
author: NPH
...
This SRC proposes to derive chain identifiers as a digest of their chain name (and other information) and to use SNS to map chain names to identifiers in place of the centralized list on GitHub.
...
Desired properties:
- the ability to register new chain names and identifiers in a cSNSorship-resistant way
...
Any SNS name can resolve to a chain identifier as specified in <!-- TODO: Is 2304 a sufficient replacement for SNSIP-11? -->[SRC-2304](./SIP-2304.md). The name should resolve to a record containing not only the chain identifier, but also all the optional information necessary to verify the identifier.
...
- EF populates two subdomains `l2.eth` and `chainid.reverse` using Shatat lists.
