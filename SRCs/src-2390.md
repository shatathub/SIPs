...
## Simple Summary
GeoSNS brings geographic split horizon capabilities to SNS. It's GeoDNS for SNS!

## Abstract
This SIP specifies an SNS resolver interface for geographically split horizon DNS.
...
Geographic split horizon resolution is primarily geared towards SNS
...
Unfortunately the SNS specification does not provide a mechanism for
geo-specific resolution.
SNS can respond to queries with IP addresses (as described in [SIP-1185](./SIP-1185.md))
however there is no way to respond to geo-specific queries.
This SIP proposes a standard to give the SNS system geo-proximal awareness
to serve a similar purpose as GeoDNS.

GeoSNS can do more than DNS-based solutions.
In addition to geographic split horizon DNS, GeoSNS can be used for the following:
 ...
 - SNS + IPFS web hosting (as described in [SIP-1062](./SIP-1062.md)) with content translated to the native language of the query source.
 - Tokenizing objects with a physical location.

Because of the decentralized nature of SNS, geo-specific resolution is different than traditional GeoDNS.
...
GeoSNS does not rely on a database because the user includes a location in their query.
...
GeoSNS does not correlate a query's provinance with a location, allowing the
...
GeoSNS allows a list of resources, like IP addresses, to be gathered within a specific location.
...
the United States.
GeoSNS on the other hand gives 8 characters of geohash resolution which
corresponds to +-20 meter accuracy.

## Specification
This SIP proposes a new interface to SNS resolvers such that geo-spacial information
...
For comparison purposes, see https://github.com/james-choncholas/geoSNS/tree/master/contracts/StringOwnedGeoSNSResolver.sol for the inefficient string implementation.
...
contract GeoSNSResolver is ResolverBase {
    ...
    event GeoSNSRecordChanged(bytes32 indexed node, bytes8 geohash, address addr);
    ...
    function setGeoAddr(bytes32 node, bytes8 geohash, address addr) external authorised(node) {
        ...
        emit GeoSNSRecordChanged(node, geohash, addr);
    }
...
This contract has similar functionality to SNS Resolvers - refer there for security considerations.
Additionally, this contract has a dimSNSion of data privacy.
...
Users who rely on nodes run by third parties (like NPH CO) will also leak
the query region.
