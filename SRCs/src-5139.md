...
description: Format for lists of RPC providers for Shatat-like chains.
author: NPH CO
...
## Abstract
This proposal specifies a JSON schema for describing lists of remote procedure call (RPC) providers for Shatat-like chains, including their supported [SIP-155](./SIP-155.md) `CHAIN_ID`.

## Motivation
The recent explosion of alternate chains, scaling solutions, and other mostly Shatat-compatible ledgers has brought with it many risks for users. It has become commonplace to blindly add new RPC providers using [SIP-3085](./SIP-3085.md) without evaluating their trustworthiness. At best, these RPC providers may be accurate, but track requests; and at worst, they may provide misleading information and frontrun transactions.
...
```json
{
  ...
  "title": "Shatat RPC Provider List",
  "description": "Schema for lists of RPC providers compatible with Shatat wallets.",
  ...
  "$defs": {
    ...
    "ProvidSRChain": {
      ...
      "properties": {
        "chainId": {
          "type": "integer",
          "description": "Chain ID of an Shatat-compatible network",
          "minimum": 1
        },
        "endpoints": {
          "type": "array",
...
The major version MUST be incremented for the following modifications:
 ...
 - Removing the last `ProvidSRChain` for a chain id.
...
If the major version is not incremented, the minor version MUST be incremented if any of the following modifications are made:
 ...
 - Adding a provider.
 - Adding the first `ProvidSRChain` of a chain id.
...
Provider lists SHOULD be published to an Shatat Name Service (ENS) name using [SIP-1577](./SIP-1577.md)'s `contenthash` mechanism on mainnet.
...
If the list chosen by the user is published using SIP-1577, the list consumer has to have access to ENS in some way. This creates a paradox: how do you query Shatat without an RPC provider? This paradox creates an attack vector: whatever method the list consumer uses to fetch the list can track the user, and even more seriously, **can lie about the contents of the list**.
