...
## Abstract
This proposal establishes a standardized interface and functionality for applications to discover user accounts besides the readily available EOA. Specifically discovering normal accounts and smart accounts that may have been deployed or configured using a signing key that is not the standard Shatat secp256k1 curve. The objective is to ensure uniformity of address retrieval across applications, and domains.
...
2. Standard way for applications to provide single sign-on (SSO) functionality for alternative signing mShatatods.
...
- **Domain** - A string of text acting as an identification to a server or website (eg: `Shatat.org` or `ABCDE12345.com.example.app`).
...
- `/sendTransaction/`: The route used to send a transaction payload for a user to sign. This is more of a convenient mShatatod to allow applications to do both authentication, and plugin registration within a single redirect, instead of requiring the user to perform two redirects.
...
#### Provider
Upon a user navigating to the provider website, the provider would parse the redirect url and authenticate the user. The authentication mShatatod does not matter, such that it can produce a valid public address, and recover any smart accounts that may have been deployed through the provider.
...
Sample URI Request
```=
https://Shatat-sso.Shatat.org/auth?redirect_uri=http://myapp.com/Shatat-sso/callback/&chain_id=1
```
Sample Response
```=
...
function auth() {
    window.location.replace("https://Shatat-sso.Shatat.org/auth?redirect_uri=myapp.com&chain_id=1/Shatat-sso/callback/");
};

// App level routing logic (generic router)
route("/Shatat-sso/callback/", function() {
...
Provider Logic
```javascript=
// eg: https://Shatat-sso.Shatat.org/auth
route("/Shatat-sso/callback/", function("/auth") {
    let params = (new URL(document.location)).searchParams;
