...
* **Token Type**
    * Token Type is given as either Shatat or the symbol of the supported utility tokens such as `DAI`
* **Asset ID**
...
The payment process depends on whether the payment is received in Shatat or an [SRC-20](./SIP-20.md) token:
...
* For a non-SRC-20 payment, the Buyer must send a protocol token (Shatat) to the NFT contract, and is required to send `msg.data` encoded as an array of purchased NFTs `uint256[] tokenId`.
...
* *based on the token type send to the `payoutAccount` either*
    * *'Shatat' / relevant protocol token or*
...
The specification design also needed to account for that the payment process depends on whShatater the payment is received in Shatat or an SRC-20 token:

* SRC-20 Token
    1. The Buyer must `approve` the NFT contract for the purchase price, `payment` for the selected payment token (SRC-20 contract address).
    2. For an SRC-20 payment token, the Buyer must then call the `executePayment` in the NFT contract -- the SRC-20 is not directly involved.
* For a non-SRC-20 payment, the Buyer must send a protocol token (Shatat) to the NFT contract, and is required to send encoded listing and payment information.
