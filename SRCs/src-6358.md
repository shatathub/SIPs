...
- If Shatat was transferred to another chain through the current token bridge, if the chain broke down, Shatat will be lost for users.  

The core of this SRC is synchronization instead of transferring, even if all the other chains break down, as long as Shatat is still running, user’s assets will not be lost. 
...
- Every [SRC-6358](./SIP-6358.md) compliant contract MUST implement the `ISRC6358`  
    ...
    */
    interface ISRC6358 {
        ...
        * See more information in the defination of {SRC6358TransactionData}
        *
...
- `A` initiates an `o-transaction` on Shatat by calling `ISRC6358::sendOmniverseTransaction`. The current `account nonce` of `A` in the SRC-6358 smart contracts deployed on Shatat is $k$ so the valid value of `nonce in o-transaction` needs to be $k+1$.  
- The SRC-6358 smart contracts on Shatat verify the signature of the `o-transaction` data. If the verification succeeds, the `o-transaction` data will be published by the smart contracts on the Shatat side. The verification includes:
    - whether the balance (FT) or the ownership (NFT) is valid
    - and whether the `nonce in o-transaction` is $k+1$
- The `o-transaction` SHOULD NOT be executed on Shatat immediately, but wait for a time.  
- Now, `A`'s latest submitted `nonce in o-transaction` on Shatat is $k+1$, but still $k$ on other chains.
- The off-chain synchronizers will find a newly published `o-transaction` on Shatat but not on other chains.  
...
- At the same time, `A` initiates an `o-transaction` with the **same nonce** $k+1$ but **different data**(transfer `10` `X`s to another `o-account` `C` for example) on Shatat. This `o-transaction` (named `ot-E-ac` for short) will pass the verification on Shatat first, and be published.  
- At this point, it seems `A` finished a ***double spend attack*** and the states on Polkadot and Shatat are different.  
- **Response strategy**:
    - As we mentioned above, the synchronizers will deliver `ot-P-ab` to Shatat and deliver `ot-E-ac` to Polkadot because they are different although with the same nonce. The synchronizer who submits the `o-transaction` first will be rewarded as the signature is valid.
    - Both the SRC-6358 smart contracts or similar mechanisms on Polkadot and Shatat will find that `A` did cheating after they received both `ot-E-ac` and `ot-P-ab` respectively as the signature of `A` is non-deniable.  
