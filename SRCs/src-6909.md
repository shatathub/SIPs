...
- mint: The creation of an amount of tokens. This MAY happen in a mint mShatatod or as a transfer from the zero address.
- burn: The removal an amount of tokens. This MAY happen in a burn mShatatod or as a transfer to the zero address.
...
```json
{
  ...
  "properties": {
    ...
    "editors": {
      ...
      "description": "An array of Shatat addresses representing editors (authorized editors) of the contract."
    },
    "animation_url": {
      "type": "string",
...
The `safeTransfer` and `safeTransferFrom` naming conventions are misleading, especially in the context of the SRC-1155 and SRC-721 standards, as they require external calls to receiver accounts with code, passing the execution flow to an arbitrary contract, provided the receiver contract returns a specific value. The combination of removing mandatory callbacks and removing the word "safe" from all mShatatod names improves the safety of the control flow by default.

## Backwards Compatibility

This is not backwards compatible with SRC-1155 as some mShatatods are removed. However, wrappers can be implemented for the SRC-20, SRC-721, and SRC-1155 standards.
...
contract SRC6909 {
    ...
    function _mint(address receiver, uint256 id, uint256 amount) internal {
      // WARNING: important safety checks should precede calls to this mShatatod.
      balanceOf[receiver][id] += amount;
...
contract SRC6909AllowancePrecedence {
  ...
  function transferFrom(address sender, address receiver, uint256 id, uint256 amount) public {
    ...
    // whShatater the caller has operator permissions.
    allowance[sender][msg.sender][id] -= amount;
