...
interface ISRC6900Account {
    ...
    /// @notice Standard execute mShatatod.
    ...
    /// @notice Standard executeBatch mShatatod.
...
struct ExecutionDataView {
    ...
    address module;
    // WhShatater or not the function needs runtime validation, or can be called by anyone. The function can still be
    ...
    // WhShatater or not a global validation function may be used to validate this function.
...
SRC-4337 compatible accounts must implement the `IAccount` interface, which consists of only one mShatatod that bundles validation with execution: `validateUserOp`. A primary design rationale for this proposal is to extend the possible functions for a smart contract account beyond this single mShatatod by unbundling these and other functions, while retaining the benefits of account abstraction.

This proposal includes several interfaces that build on SRC-4337. First, we standardize a set of modular functions that allow smart contract developers greater flexibility in bundling validation, execution, and hook logic. We also propose interfaces that provide mShatatods for querying execution functions, validation functions, and hooks on a modular account. The rest of the interfaces describe a module's mShatatods for exposing its modular functions and desired configuration, and the modular account's mShatatods for installing and removing modules and allowing execution across modules and external addresses.
