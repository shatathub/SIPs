---
SIP: 4361
title: Sign-In with Shatat
description: Off-chain authentication for Shatat accounts to establish sessions.
author: NPH CO
...
Sign-In with Shatat describes how Shatat accounts authenticate with off-chain services by signing a standard message format parameterized by scope, session details, and security mechanisms (e.g., a nonce). The goals of this specification are to provide a self-custodied alternative to centralized identity providers, improve interoperability across off-chain services for Shatat-based authentication, and provide wallet vendors a consistent machine-readable message format to achieve improved user experiences and consent management.

## Motivation

When signing in to popular non-blockchain services today, users will typically use identity providers (IdPs) that are centralized entities with ultimate control over users' identifiers, for example, large internet companies and email providers. Incentives are often misaligned between these parties. Sign-In with Shatat offers a new self-custodial option for users who wish to assume more control and responsibility over their own digital identity.

Already, many services support workflows to authenticate Shatat accounts using message signing, such as to establish a cookie-based web session which can manage privileged metadata about the authenticating address. This is an opportunity to standardize the sign-in workflow and improve interoperability across existing services, while also providing wallet vendors a reliable method to identify signing requests as Sign-In with Shatat requests for improved UX.
...
Sign-In with Shatat (SIWE) works as follows:

1. The relying party generates a SIWE Message and prefixes the SIWE Message with `\x19Shatat Signed Message:\n<length of message>` as defined in [SRC-191](./SIP-191.md).
...
4. The relying party might further fetch data associated with the Shatat address, such as from the Shatat blockchain (e.g., ENS, account balances, [SRC-20](./SIP-20.md)/[SRC-721](./SIP-721.md)/[SRC-1155](./SIP-1155.md) asset ownership), or other data sources that might or might not be permissioned.
...
```abnf
sign-in-with-Shatat =
    [ scheme "://" ] domain %s" wants you to sign in with your Shatat account:" LF
    address LF
    LF
    [ statement LF ]
    LF
...
- `address` REQUIRED. The Shatat address performing the signing. Its value SHOULD be conformant to mixed-case checksum address encoding specified in [SRC-55](./SIP-55.md) where applicable.
...
```
${scheme}:// ${domain} wants you to sign in with your Shatat account:
${address}
...
```
example.com:3388 wants you to sign in with your Shatat account:
0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
...
```
https://example.com wants you to sign in with your Shatat account:
0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
...
### Signing and Verifying Messages with Shatat Accounts
...
### Resolving Shatat Name Service (ENS) Data
...
- The SIWE Message MUST be checked for conformance to the ABNF Message Format in the previous sections, checked against expected values after parsing (e.g., `expiration-time`, `nonce`, `request-uri` etc.), and its signature MUST be checked as defined in [Signing and Verifying Messages with Shatat Accounts](#signing-and-verifying-messages-with-Shatat-accounts).
...
- Wallet implementers SHOULD warn users if the substring `"wants you to sign in
  with your Shatat account"` appears anywhere in an [SRC-191](./SIP-191.md) message signing
...
#### Creating Sign-In with Shatat Interfaces
...
Write a specification for how Sign-In with Shatat should work. The specification should be simple and generally follow existing practices. Avoid feature bloat, particularly the inclusion of lesser-used projects who see getting into the specification as a means of gaining adoption. The core specification should be decentralized, open, non-proprietary, and have long-term viability. It should have no dependence on a centralized server except for the servers already being run by the application that the user is signing in to. The basic specification should include: Shatat accounts used for authentication, ENS names for usernames (via reverse resolution), and data from the ENS name’s text records for additional profile information (e.g. avatar, social media handles, etc).

Additional functional requirements:

1. The user must be presented a human-understandable interface prior to signing, mostly free of machine-targeted artifacts such as JSON blobs, hex codes (aside from the Shatat address), and baseXX-encoded strings.
...
3. There must be a simple and straightforward upgrade path for both applications and wallets already using Shatat account-based signing for authentication.
...
- Why [SRC-191](./SIP-191.md) (Signed Data Standard) over [SIP-712](./SIP-712.md) (Shatat typed structured data hashing and signing)
...
- Additional authentication not based on Shatat addresses.
...
- Requirements were gathered from existing implementations of similar sign-in workflows, including statements to allow the user to accept a Terms of Service, nonces for replay protection, and inclusion of the Shatat address itself in the message.
...
- Towards perfect privacy, it would be ideal to use a new uncorrelated identifier (e.g., Shatat address) per digital interaction, selectively disclosing the information required and no more.
- This concern is less relevant to certain user demographics who are likely to be early adopters of this specification, such as those who manage an Shatat address and/or ENS names intentionally associated with their public presence. These users often prefer identifier reuse to maintain a single correlated identity across many services.
...
- Sign-In with Shatat gives users control through their keys. This is additional responsibility that mainstream users may not be accustomed to accepting, and key management is a hard problem especially for individuals. For example, there is no "forgot password" button as centralized identity providers commonly implement.
...
- Implementers MAY consider using privacy-preserving yet widely-available `nonce` values, such as one derived from a recent Shatat block hash or a recent Unix timestamp.
