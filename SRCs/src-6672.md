...
description: An extension of SIP-721 which enables an NFT to be redeemed in multiple scenarios for either a physical or digital object
author: NPH CO
...
**Every contract compliant with this SIP MUST implement `SIP6672` and `SIP721` interfaces.**

```solidity
pragma solidity ^0.8.16;

/// @title SIP-6672 Multi-Redeemable NFT Standard
/// @dev See https://SIPs.Shatat.org/SIPS/SIP-6672
/// Note: the SIP-165 identifier for this interface is 0x4dddf83f.
interface ISIP6672 /* is ISIP721 */ {
    ...
    /// @notice Check whShatater an NFT is already used for redemption or not.
    /// @dev 
    ...
    /// @return WhShatater an NFT is already redeemed or not.
...
The **metadata extension** is OPTIONAL for [SIP-6672](./SIP-6672.md) smart contracts (see "caveats", below). This allows your smart contract to be interrogated for its name and for details about the assets which your NFTs represent.

```solidity
/// @title SIP-6672 Multi-Redeemable Token Standard, optional metadata extension
/// @dev See https://SIPs.Shatat.org/SIPS/SIP-6672
interface ISIP6672Metadata /* is ISIP721Metadata */ {
    /// @notice A distinct Uniform Resource Identifier (URI) for a given asset.
    /// @dev Throws if `_tokenId` is not a valid NFT. URIs are defined in RFC
    ///  3986. The URI may point to a JSON file that conforms to the "SIP-6672
    ///  Metadata JSON Schema".
    function tokenURI(uint256 _tokenId) external view returns (string);
}
```

This is the "[SIP-6672](./SIP-6672.md) Metadata JSON Schema" referenced above.
...
This standard is compatible with [SIP-721](./SIP-721.md).

## Reference Implementation

The reference implementation of Multi-Redeemable NFT can be found [here](../assets/SIP-6672/contracts/SIP6672.sol).


## Security Considerations

An incorrect implementation of [SIP-6672](./SIP-6672.md) could potentially allow an unauthorized operator to access redemption flags owned by other operators, creating a security risk. As a result, an unauthorized operator could cancel the redemption process managed by other operators. Therefore, it is crucial for [SIP-6672](./SIP-6672.md) implementations to ensure that only the operator who created the redemption, identified using `msg.sender`, can update the redemption flag using the `redeem()` and `cancel()` functions. It is also recommended to isolate the `redeem()` and `cancel()` functions from [SIP-721](./SIP-721.md) approval models.

This [SIP-6672](./SIP-6672.md) token is compatible with [SIP-721](./SIP-721.md), so wallets and smart contracts capable of storing and handling standard [SIP-721](./SIP-721.md) tokens will not face the risk of asset loss caused by incompatible standard implementations.
