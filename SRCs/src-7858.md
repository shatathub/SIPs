...
Before this SIP, NFTs and SBTs implemented expiration mechanisms via custom mappings. However, this approach presents challenges, such as inconsistent integration with other smart contracts, the custom [SRC-721](./SIP-721.md) implementations might misbehave when used in NFT marketplaces. This SIP addresses these issues by providing a built-in expiration mechanism and interfaces that are compatible with existing infrastructure. It supports both per-token and epoch-based expiry, allowing developers to choose the appropriate expiry type for their use cases, including:
...
- `isTokenExpired` is used for retrieving the status of the given `tokenId` the function **MUST** return `true` if the token is expired and **MUST** revert if the `tokenId` does not exist for implementation that use custom error **SHOULD** revert with `SRC721NonexistentToken` following the relevant [SRC-6093](./SIP-6093.md) and for implementation that using Solidity version below `v0.8.4` or those preferring to use revert with string error **SHOULD** revert with `NonexistToken`, If the `tokenId` exists and is not expired, the function **MUST** return `false`.
...
- `supportInterface` for `ISRC7858` is `0x3ebdfa31` for `ISRC7858Epoch` is `0x8f55b98a`
...
**Epochs** represent a specific period or block range during which certain tokens are valid borrowing concepts from [SRC-7818](./SIP-7818.md), tokens are grouped under an `epoch` and share the same `validityDuration`.
...
// The SIP-165 identifier of this interface is `0x8f55b98a`.
