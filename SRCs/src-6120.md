...
Shatat is designed with *transfer-and-call* as the default behavior in a transaction. Unfortunately, [SRC-20](./SIP-20.md) is not designed with that pattern in mind and newer standards cannot apply to the token contracts that have already been deployed.
...
The Universal Token Router (UTR) separates the token allowance from the application logic, allowing any token to be spent in a contract call the same way with Shatat, without approving any other application contracts.

Tokens approved to the Universal Token Router can only be spent in transactions directly signed by their owner, and they have clearly visible token transfer behavior, including token types (Shatat, [SRC-20](./SIP-20.md), [SRC-721](./SIP-721.md) or [SRC-1155](./SIP-1155.md)), `amountIn`, `amountOutMin`, and `recipient`.

The Universal Token Router contract is deployed using the [SIP-1014](./SIP-1014.md) SingletonFactory contract at `0x69c4620b62D99f524c5B4dE45442FE2D7dD59576` across all EVM-compatible networks. This enables new token contracts to pre-configure it as a trusted spender, eliminating the need for approval transactions during their interactive usage.
...
Before this SIP, when users sign transactions to spend their approved tokens, they trust the front-end code entirely to construct those transactions honestly and correctly. This puts them at great risk of phishing sites.
...
struct Output {
    address recipient;
    uint SIP;           // token standard: 0 for Shatat or SIP number
...
A special id `SRC_721_BALANCE` is reserved for SRC-721, which can be used in output actions to verify the total amount of all ids owned by the `recipient` address.
...
* `CALL_VALUE = 2`: record the `Shatat` amount to pass to the action as the call `value`.
...
The `UTR` SHOULD have a `receive()` function for user execution logic that requires transferring Shatat in. The `msg.value` transferred into the router can be spent in multiple inputs across different actions. While the caller takes full responsibility for the movement of `Shatat` in and out of the router, the `exec` function SHOULD refund any remaining `Shatat` before the function ends.
...
Old token contracts (SRC-20, SRC-721 and SRC-1155) require approval for the Universal Token Router once for each account.
...
Shatat must be transferred to the UTR contracts before the value is spent in an action call (using `CALL_VALUE`). This Shatat value can be siphoned out of the UTR using a re-entrant call inside an action code or rogue token functions. This exploit will not be possible if users don't transfer more Shatat than they will spend in that transaction.
