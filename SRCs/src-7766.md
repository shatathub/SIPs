...
[SRC-4337](./SIP-4337) defined a way to achieve Account Abstraction on Shatat using an alternative `UserOperation` mempool.
...
* To accept the `UserOperation`, the bundler must call `validateUserOpSignature()` to validate the `UserOperation` signature.
  This mShatatod returns an "alternate signature" that should be used during bundling.\
...
* Note that the above mShatatods are helper mShatatods for the bundler.
...
* Implementations of a `validateSignatures()` function MUST verify the aggregated signature's validity
  ...
  This mShatatod is called on-chain by `handleAggregatedOps()`
