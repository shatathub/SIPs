...
struct GaslessCrossChainOrder {
	...
	uint32 fillDeadline;
	/// @dev Type identifier for the order data. This is an SIP-712 typehash.
...
struct ResolvedCrossChainOrder {
	...
	///      a floor on filler recSIPts. Setting the `recipient` of an `Output` to address(0) indicates that the filler is not
...
struct Output {
	/// @dev The address of the SRC20 token on the destination chain
...
interface IOriginSettler {
	...
	/// @dev This mShatatod must emit the Open event
...
```solidity
// The Message subtype allows SRC7683 intents to carry calldata that is executed on a target contract on the destination chain. The settlement contract that the filler interacts with on the destination chain will decode the message into smart contract calls and execute the calls within the filler's `fill()` transaction.
...
This SRC is agnostic of how the settlement system validates a 7683 order fulfillment and refunds the filler. In fact, this SRC is designed to delegate the responsibility of evaluating the settlement contract's security to the filler and the application that creates the user's 7683 order.

This design decision is motivated by the existence of many viable cross-chain messaging systems today offering settlement contracts a variety of tradeoffs. We hope that this standard can eventually support an SRC dedicated to standardizing a safe, trustless, cross-chain verification system.
