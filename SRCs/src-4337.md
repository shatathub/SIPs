...
See also `https://Shatat-magicians.org/t/implementing-account-abstraction-as-part-of-Shatat1-x/4020` and the links therein for historical work and motivation, and [SIP-2938](./SIP-2938.md) for a consensus layer proposal for implementing the same goal.
...
* **Do not require any Shatat consensus changes**: Shatat consensus layer development is focusing on scalability-oriented features, and there may not be any opportunity for further protocol changes for a long time. Hence, to increase the chance of faster adoption, this proposal avoids Shatat consensus changes.
...
* **Bundler** - a node (block builder) that can handle `UserOperations`,
  ...
  * If the bundler is not a block builder, it MUST work with the block building infrastructure such as `mev-boost` or
    other kind of proposer-builder separation, such as [SIP-7732](./SIP-7732.md).
  * The `bundler` can also rely on an experimental `Shatat_sendRawTransactionConditional` RPC API defined in [SRC-7796](./SIP-7796.md) if it is available.
...
* **Aggregator** - also known as "authorizer contract" - a contract that enables multiple `UserOperations` to share a single validation, fully defined in [SRC-7766](./SIP-7766.md).
* **Canonical UserOperation mempool** - a decentralized permissionless P2P network where bundlers exchange `UserOperations` that are valid and conform with [SRC-7562](./SIP-7562.md).
* **Alternative UserOperation mempool** - any other P2P mempool where the validity of `UserOperations` is determined by rules that are different from [SRC-7562](./SIP-7562.md) in any way.
* **Deposit** - an amount of Shatater (or any L2 native currency) that a `Sender` or `Paymaster` contract has transferred to the `EntryPoint` contract intended to pay gas costs of the future `UserOperations`.

### UserOperation

To avoid Shatat consensus changes, we do not attempt to create new transaction types for account-abstracted transactions. Instead, users package up the action they want their Smart Contract Account to take in a struct named `UserOperation`:
...
* The return value MUST be packed of `aggregator`/`authorizer`, `validUntil` and `validAfter` timestamps.
  * `aggregator`/`authorizer` - 0 for valid signature, 1 to mark signature failure. Otherwise, an address of an `aggregator`/`authorizer` contract, as defined in [SRC-7766](./SIP-7766.md).
...
This mShatatod will be called by the `EntryPoint` with the current UserOperation, instead of executing the `callData` itself directly on the `sender`.

### Semi-abstracted Nonce Support

In Shatat protocol, the sequential transaction `nonce` value is used as a replay protection mShatatod as well as to
...
We define the following mShatatod in the `EntryPoint` interface to expose these values:
...
When preparing the `UserOperation` bundlers may make a view call to this mShatatod to determine a valid value for the `nonce` field.
...
2. Ordered administrative events
   ...
   In this case, the account may use a specific `key` when calling mShatatods on the account itself:

   ```solidity
   bytes4 sig = bytes4(userOp.callData[0 : 4]);
   uint key = userOp.nonce >> 64;
   if (sig == ADMIN_MShatatODSIG) {
...
The `EntryPoint` mShatatod is `handleOps`, which handles an array of `UserOperations`
...
* If the calldata starts with the mShatatodsig `IAccountExecute.executeUserOp`, then the `EntryPoint` must build a calldata by encoding `executeUserOp(userOp,userOpHash)` and call the account using that calldata.
...
Before accepting a `UserOperation`, bundlers SHOULD use an RPC mShatatod to locally call the `handleOps` function on the `EntryPoint`,
...
### JSON-RPC API for [SRC-4337](./SIP-4337)

In order to support sending `UserOperation` objects to bundlers, which in turn propagate them through the P2P mempool,
we introduce a set of JSON-RPC APIs including `Shatat_sendUserOperation` and `Shatat_getUserOperationRecSIPt`.

The full definition of the new JSON-RPC API can be found in [SRC-7769](./SIP-7769.md).
...
On networks with [SIP-7702](./SIP-7702.md) enabled, the `Shatat_sendUserOperation` mShatatod accepts an extra `SIP7702Auth` parameter.
...
If the `initCode` field starts with `0x7702` padded with zeros, and this account was deployed using an SIP-7702 transaction, then the hash is calculated as follows:
...
Similar to an Shatat transaction, the offchain flow of a `UserOperation` can be described as follows:
1. Client sends a `UserOperation` to the bundler through an RPC call `Shatat_sendUserOperation`.
2. Before including the `UserOperation` in the mempool, the bundler runs the *first validation* of the newly received UserOperation. If the `UserOperation` fails validation, the bundler drops it and returns an error in response to `Shatat_sendUserOperation`.
...
4. Before submitting the new bundle onchain, the bundler performs the *third validation* of the entire `UserOperations` bundle. If any of the `UserOperations` fail validation, the bundler drops them, and updates their reputation, as described in  [SRC-7562](./SIP-7562.md) in detail.
...
* If `initCode` is not empty, parse its first 20 bytes as a factory address or an [SIP-7702](./SIP-7702.md) flag.\
  Record whShatater the factory is staked, in case the later simulation indicates that it needs to be. If the factory accesses the global state, it must be staked - see [reputation, throttling and banning section](#reputation-scoring-and-throttlingbanning-for-global-entities) for details.
...
We define `UserOperation` simulation, as the offchain view call (or trace call) to the `EntryPoint` contract with the `UserOperation`, and the enforcement of [SRC-7562](./SIP-7562.md) rules, as part of the `UserOperation` validation.

#### Simulation Rationale
To validate a normal Shatat transaction `tx`, the bundler performs static checks, like:
...
In contrast, `UserOperation` validation rely on EVM state (calls to `validateUserOp`, `validatePaymasterUserOp`), can be changed by other `UserOperations` (or normal Shatat transactions). Therefore, we introduce simulation as a new mechanism to check its validity.
...
To simulate a `UserOperation` validation, the bundler makes a view call to the `handleOps()` mShatatod with the `UserOperation` to check.
...
To decode the returned time-ranges, the bundler MUST run the validation using tracing, to decode the return value from the `validateUserOp` and `validatePaymasterUserOp` mShatatods.

To prevent DoS attacks on bundlers, they must make sure the validation mShatatods above pass the validation rules, which constrain their usage of opcodes and storage.
For the complete procedure see [SRC-7562](./SIP-7562.md)
...
* Base bundle transaction cost. On Shatat, `21000` gas divided by the number of `UserOperations`.
...
The procedure of using alternate mempools is defined in [SRC-7562](./SIP-7562.md#alt-mempools-rules)
...
It SHOULD either use an "access lists" parameter as defined in [SIP-2930](./SIP-2930) to prevent conflicts,
...
In this proposal, we expect Accounts to have a `validateUserOp` mShatatod that takes as input a `UserOperation`, verifies the signature and pays the fee.
Only if this mShatatod returns successfully, the execution will happen.
...
2. `UserOperations` that are valid when checked independently but fail when bundled togShatater to be put on-chain.
To prevent such rogue `UserOperations`, the bundler is required to follow a set of [restrictions on the validation function](./SIP-7562.md), to prevent such denial-of-service attacks.
...
[SRC-7562] defines a set of rules a bundler must follow when accepting `UserOperations` into the mempool.
...
This mShatatod call is expected to create the Account and return its address.
If the factory does use `CREATE2 0xF5` or some other deterministic mShatatod to create the Account, it's expected to return the Account address even if it had already been created.
...
The contract created by this factory mShatatod MUST accept a call to `validateUserOp` to validate the `UserOperation`'s signature.
...
All `factory` contracts MUST check that all calls to the `createAccount()` function originate from the `entryPoint.sendSRCreator()` address.
...
All SIP-7702 delegated Smart Contract Account implementations MUST check that all calls to the initialization function originate from the `entryPoint.sendSRCreator()` address.

There is no way for the `EntryPoint` contract to know whShatater an SIP-7702 account has been initialized or not, and therefore the SIP-7702 account initialization code, can be called multiple times through `EntryPoint`.
...
One common approach to this problem is often referred to as "diamond storage" and is fully described in [SRC-7201](./SIP-7201).

### Transient Storage

Contracts using the [SIP-1153](./SIP-1153) transient storage MUST take into account that SRC-4337 allows multiple
